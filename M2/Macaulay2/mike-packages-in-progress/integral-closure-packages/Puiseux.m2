...
TEST ///
  restart
  debug loadPackage "Puiseux"
  R = QQ[x,y]
  F = x^3*y^2 + 3 * x*y^4 + x^2*y^4 + y^7 + x^12*y + x^15
  polygon1(F,true)
  polygon1(F,false)
  A = QQ[t]
  polygon(F,t)
...
doc ///
   Key
     "Puiseux Example"
   Headline
     Example of use of Puiseux series
   Description
    Text
      The following example, found by Rob Koelman, appears in
      the seminal paper of Mark van Hoeij (J. Symbolic Computation, 1995, 11)
      (although, it appears that the polynomial there is not correct!)
    Example
      R = QQ[x,y]
      F = poly"y10+(-2494x2+474)y8+(84366+2042158x4-660492x2)y6
	       ...
	       +34263110700x4-228715574724x6+5431439286x2
           +201803238-9127158539954x10-3212722859346x8"
      F1 = (761328152*x^6*z^4-5431439286*x^2*y^8+2494*x^2*z^8+
      ...
      F = (761328152*x^6-5431439286*x^2*y^8+2494*x^2+
        228715574724*x^6*y^4+9127158539954*x^10-15052058268*x^6*y^2+
        3212722859346*x^8*y^2-134266087241*x^8-202172841*y^8
        -34263110700*x^4*y^6-6697080*y^6-2042158*x^4-201803238*y^10+
        12024807786*x^4*y^4-128361096*x^4*y^2+506101284*x^2*y^6+
        47970216*x^2*y^4+660492*x^2*y^2-1-474*y^2-84366*y^4);
    Text
      $F$ defines a curve of degree 10 in the plane.  Its arithmetic genus is 36.
      Let's analyze this curve.  It is supposed to have 26 cusps.
    Example
      singF = ideal F + ideal jacobian ideal F;
      ideal gens gb singF
      decompose oo
    Text
      Hmmm, this doesn't seem to have many singular points.
      Let's compute the discriminant.
    Example
      D = discriminant(F,y);
      facsD = (factor D)//toList/toList
      facsD = select(facsD, v -> v#1 > 1)
    Text
      Singular points must all be at x-values which occur with 
      at least an exponent 2 in the discriminant.
    Example
      ...
      P_1_1
    Text
      The first Puiseux series has multiplicity 3, and represents 2 series
      for a total of 6 puiseux factors of F.
      
      The second series has multiplicity 1, but is defined over a field
      extension of degree 4, so this really represents 4 smooth branches above
      x=0.
      
      The total is 10, as expected.
    Text
      The map $V(F) \longarrow P^1$ projecting onto the $x$ coordinate
      is a degree 10 map.  
    Example
      puiseux(F, 10, Center=>49*x^2+16)
      Ps = puiseux(F, 10, Center=>196*x^2+1)
      Ps_0_1 -- 2 x-values being considered: the two conjugates of 196*x^2+1 = 0.
             -- 6 smooth points each.
      Ps_1_1 -- a single cusp for each of these x-values, mult 2.
      Ps_2_1 -- another cusp, mult 2.
    Example
      Ps = puiseux(F, 10, Center=>299*x^2-1)
      ring Ps_0_1 -- point of mult 2 (2 cusps).
      ring Ps_1_1 -- 8 smooth points in each fibers 299*x^2=1.
    Example
      Ps = puiseux(F, 10, Center=>676*x^2+9)
      ring Ps_0_1 -- 8 smooth points in each fibers 299*x^2=1.
      ring Ps_1_1 -- point of mult 1, but tangent.
  SeeAlso
///
